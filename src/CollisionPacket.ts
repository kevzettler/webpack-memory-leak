...
export default class CollisionPacket{
  ...
  @observable r3Position = vec3.create();
  @observable r3Velocity = vec3.create();

  //Information about the move being requested in eSpace
  @observable eIntersectionPoint = vec3.create()
  ...
  @computed({keepAlive: true}) get ePosition(){
  ...
  @computed({keepAlive: true}) get eDestination(){
    return vec3.add(
      vec3.create(),
      this.ePosition,
      this.eVelocity
  ...
  @computed get eVelSquaredLen(){
    return vec3.squaredLength(this.eVelocity);
  }

  @computed({keepAlive: true}) get eVelLen(){
    return vec3.length(this.eVelocity);
  }

  @computed get eNormalizedVelocity(){
  ...
  @computed({keepAlive: true}) get distanceToCollision(){
    return this.t * this.eVelLen;
  }

  @computed({keepAlive: true}) get eNearestNonIntersectPosition(){
  ...
  @computed({keepAlive: true}) get SlidingPlane(){
  ...
  @computed({keepAlive: true}) get eSlideDestination(){
    /*
  ...
  @action reset(position:vec3, velocity:vec3){
  ...
  @action setCollision(t:number, interesctionPoint: vec3){
