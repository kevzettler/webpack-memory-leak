...
export default class NetworkBase {
  ...
  constructor(dispatchProxy: Function){
    ...
    this.signalhub = signalHub('crashgiants', [
      config.signalHub.url
    ]);
    this.config = config;
  }

  @action networkPeerCreate(
    id: string,
    protocol: string,
    peer: IPeer,
  ){
  ...
  @action networkPeerConnect(id: string, protocol:string, status="sync"){
    this.peers[id].status = status;
    this.peers[id].protocol = protocol;
  }

  @action networkPeerClose(peerId: string){
  ...
  @action networkPeerError({offerId}: {offerId: string}){
  ...
  @action peerCloseHandler(peerId: string){
